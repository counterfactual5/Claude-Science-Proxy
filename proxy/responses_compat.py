...
def max_output_tokens(req, model, state, has_tools, rule_ids=None):
    return provider_policy.clamp_max_tokens(req.get("max_tokens"), model, state)
...
def map_tools(tools, rule_ids=None):
    out = []
    for t in tools or []:
        name = t.get("name")
        if not name:
...
def anthropic_to_openai(req, state):
    out, _metadata = anthropic_to_openai_with_metadata(req, state)
    return out


def anthropic_to_openai_with_metadata(req, state):
    rule_ids = []
    ...
    tools = map_tools(req.get("tools"), rule_ids)
