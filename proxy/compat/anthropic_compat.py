...
def build_csp_web_access_guidance(now=None):
    ...
    return (
        ...
        "  GENERAL (news/products/\"latest models\"/facts):\n"
        "    data = host.mcp(\"web-search\", \"web_search\", query=\"...\", max_results=N)\n"
        "    # csp_web_search is an ALIAS of web_search (same method, not a 2nd engine)\n"
        "    # auto: optional Brave/Serper/Tavily IF keyed → duckduckgo_ia → "
        "duckduckgo_lite → wikipedia (no key required)\n"
        "  LITERATURE (papers/DOI/scholarly):\n"
        ...
        "- For product/news queries use web_search (or its alias csp_web_search) — NOT "
        "search_literature (that lane is academic-only).\n"
        "- DuckDuckGo Instant Answer needs no API key. Empty IA / "
        "\"duckduckgo_ia: no results\" is common for news queries and does NOT mean "
        "keys are missing — free duckduckgo_lite/wikipedia follow automatically. "
        "Do NOT tell the user they must configure Brave/Serper/Tavily; those keys "
        "are optional reliability upgrades only.\n"
