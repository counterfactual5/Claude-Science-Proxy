...
def build_csp_web_access_guidance(now=None):
    ...
    return (
        ...
        "    data = host.mcp(\"web-search\", \"search_literature\", query=\"...\", "
        "max_results=N)\n"
        ...
        "- provider=\"auto\" (default) tries keyed Brave/Serper/Tavily first when "
        "configured, then general no-key sources (duckduckgo_ia, wikipedia), then "
        "scholarly (Crossref/arXiv/PubMed). For product/news/\"latest\" queries use "
        "auto or provider=\"duckduckgo_ia\" — do NOT assume results are only academic "
        "papers. For literature-only, set provider=\"crossref\" (or arxiv/pubmed).\n"
