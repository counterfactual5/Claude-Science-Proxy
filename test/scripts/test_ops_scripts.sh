...
DOCTOR="$ROOT/scripts/maintenance/doctor.sh"
VERIFY="$ROOT/scripts/ci/verify-proxy.sh"
SELFTEST="$ROOT/scripts/ci/self-test.sh"
CLEAN="$ROOT/scripts/ci/clean-bundle-resources.sh"
PROXY="$ROOT/proxy/core/csp_proxy.py"
T="$(mktemp -d)"
cleanup_bundle_test_artifacts() {
  rm -f "$ROOT/proxy/core/__pycache__/csp-clean-test.pyc" "$ROOT/scripts/ci/csp-clean-test.pyc"
  rm -f "$ROOT/proxy/core/csp-clean-test.pyo" "$ROOT/scripts/ci/csp-clean-test.pyo"
  rmdir "$ROOT/proxy/core/__pycache__" 2>/dev/null || true
}
...
if [ "$(python3 "$ROOT/test/fixtures/_capability.py")" != "1" ]; then
  echo "skip - doctor occupied-port classification env-blocked (loopback disabled)"
else
...
if [ "$(python3 "$ROOT/test/fixtures/_capability.py")" != "1" ]; then
  echo "skip - verify-proxy segment env-blocked (loopback disabled, cannot start temp proxy)"
else
...
PYTHONPATH="$ROOT" DEEPSEEK_API_KEY=fake CSP_UPSTREAM_URL="http://127.0.0.1:1/never" \
...
mkdir -p "$ROOT/proxy/core/__pycache__"
: > "$ROOT/proxy/core/__pycache__/csp-clean-test.pyc"
: > "$ROOT/scripts/ci/csp-clean-test.pyc"
: > "$ROOT/proxy/core/csp-clean-test.pyo"
: > "$ROOT/scripts/ci/csp-clean-test.pyo"
out="$("$CLEAN" 2>&1)"; rc=$?
if [ $rc -eq 0 ]; then ok "clean-bundle-resources exits 0"; else no "clean-bundle-resources failed (rc=$rc): $out"; fi
if [ ! -e "$ROOT/proxy/core/__pycache__/csp-clean-test.pyc" ] \
  && [ ! -e "$ROOT/scripts/ci/csp-clean-test.pyc" ] \
  && [ ! -e "$ROOT/proxy/core/csp-clean-test.pyo" ] \
  && [ ! -e "$ROOT/scripts/ci/csp-clean-test.pyo" ]; then
