...
out="$(HOME="$OUTER_HOME" SANDBOX_HOME="$T/home" SCIENCE_BIN="$FAKE_FAIL" "$ROOT/scripts/sandbox/stop-science-sandbox.sh" 2>&1)"; rc=$?
...
out="$(HOME="$OUTER_HOME" SANDBOX_HOME="$T/home" SCIENCE_BIN="$FAKE_OK" "$ROOT/scripts/sandbox/stop-science-sandbox.sh" 2>&1)"; rc=$?
...
out="$(HOME="$T/realhome" SANDBOX_HOME="$T/realhome" SCIENCE_BIN="$FAKE_OK" "$ROOT/scripts/sandbox/stop-science-sandbox.sh" 2>&1)"; rc=$?
...
out="$(HOME="$OUTER_HOME" SANDBOX_HOME="$T/linkhome" SCIENCE_BIN="$FAKE_OK" "$ROOT/scripts/sandbox/stop-science-sandbox.sh" 2>&1)"; rc=$?
...
out="$(SANDBOX_HOME="$T/vh" "$ROOT/scripts/sandbox/launch-virtual-sandbox.sh" --port 08765 --dry-run 2>&1)"; rc=$?
if [ $rc -ne 0 ] && echo "$out" | grep -q "拒绝"; then ok "08765 rejected via int-normalize"; else no "08765 bypassed guard (rc=$rc)"; fi

out="$(SANDBOX_HOME="$T/vh" "$ROOT/scripts/sandbox/launch-virtual-sandbox.sh" --port 9931 --dry-run 2>&1)"; rc=$?
...
out="$(SANDBOX_HOME="$T/vh2" "$ROOT/scripts/sandbox/launch-virtual-sandbox.sh" --port 8765x --dry-run 2>&1)"; rc=$?
