...
class InjectCspWebAccessGuidance(unittest.TestCase):
    ...
    def test_appends_to_string_system(self):
        ...
        self.assertIn('host.mcp("web-search", "csp_web_search"', out["system"])
        ...
        self.assertIn("fell back to Wikipedia", out["system"])
        ...
        self.assertIn("Do NOT call bare `web_search`", out["system"])
        self.assertIn("search_skills", out["system"])
        self.assertIn("Missing 'query' argument", out["system"])
        self.assertIn('prefix="mcp-"', out["system"])
