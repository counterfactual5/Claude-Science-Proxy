...
> **迁移注记（2026-07-10）**：`prepare-legacy` 在隔离 HOME 写入 **v1 单槽 DeepSeek** 样本（仅 `DEEPSEEK_API_KEY`）；首启应迁移为一条 DeepSeek profile。第二条 profile 切换（RM-05）须在面板 **＋ 新建**（如 GLM）。详见 `test/RM_RETEST_STEPS.md`。
...
- 若已安装的 CSP 正在运行，用 `test/tauri.real-machine.conf.json` 构建独立
