...
前置：DeepSeek 生效、代理与沙箱在跑（先点一次「启动 Claude Science」）。记下代理 PID 和 `active_id`。

1. 在面板把 **Qwen**（非当前生效那条）的 key 改成 `$BAD_KEY` 保存。
2. **点击 Qwen 配置卡片**切换为当前生效。
...
- 已装的 `/Applications/Claude Science Proxy.app`（或旧名 `CSSwitch.app`）原进程没被退出（验收 app 是独立 bundle id，并行跑）。
