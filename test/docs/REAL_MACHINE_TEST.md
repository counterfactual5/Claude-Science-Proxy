...
> **迁移注记（2026-07-10）**：`prepare-legacy` 在隔离 HOME 写入 **v1 单槽 DeepSeek** 样本（仅 `DEEPSEEK_API_KEY`）；首启应迁移为一条 DeepSeek profile。第二条 profile 切换（RM-05）须在面板 **＋ 新建**（如 GLM）。详见 `test/docs/RM_RETEST_STEPS.md`。
...
- 若已安装的 CSP 正在运行，用 `test/fixtures/tauri.real-machine.conf.json` 构建独立
...
机器上没有其它 CSP 进程时，可直接启动当前工作树 release 二进制：
...
机器上已有 CSP 时，不要为测试强退用户实例。构建并启动独立验收 app：

```bash
cd desktop
PATH="$HOME/.rustup/toolchains/stable-aarch64-apple-darwin/bin:$PATH" \
  npm run tauri build -- --config ../test/fixtures/tauri.real-machine.conf.json --bundles app

HOME="$(cd .. && bash test/real_machine_guard.sh env | sed -n 's/^HOME=//p')" \
CSP_REPO="$(cd .. && pwd)" \
src-tauri/target/release/bundle/macos/CSP\ Acceptance.app/Contents/MacOS/desktop
```
...
| ID | 场景 | 操作 | 必须满足 |
|---|---|---|---|
| RM-01 | v1→v2 迁移 | 用 `prepare-legacy` 后首次启动 | 列表出现 **DeepSeek**（当前生效）；`CSP.json.v1.bak` 存在且 0600；key 只显示掩码 |
...
| RM-05 | 激活切换 | DeepSeek↔新建的第二条 profile（如 GLM） | scratch→正式代理健康后才切 active；代理 PID/adapter 变化；沙箱 PID 不变；请求走新上游 |
...
| RM-15 | 全部停止/退出 | 点「全部停止」，再退出 app | 据实报告停止结果；测试端口释放；无残留 CSP/python 子进程 |
| RM-16 | 重启恢复 | 退出并用同一测试 HOME 重开 | profiles/active/notes/端口持久；不自动启动代理或沙箱 |
| RM-17 | 包资源 | 从 `.app` 与挂载 DMG 启动 | `Contents/Resources/{proxy,scripts}` 齐全；无需 `CSP_REPO` 也能起代理 |
