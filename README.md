...
[下载最新版](../../releases/latest) · [更新日志](./CHANGELOG.md)
...
1. 从 [GitHub Releases](../../releases/latest) 下载最新的 `Claude Science Proxy_*.dmg`。
...
5. 点击「创建」保存配置。
6. 在配置列表中点击「设为当前」来激活 profile；可以同时激活多条 profile（provider pool 模式）。
7. 验证通过后点击「一键开始」。
8. CSP 会启动隔离 Science，并在浏览器中打开入口；Science 模型选择器里会显示你配置的真实模型名。

## 虚拟模型注册表（multi-model）

Science 只认 `claude-` 开头的模型 ID。CSP 内置了一个虚拟模型注册表，从固定的 `claude-opus-*` / `claude-sonnet-*` / `claude-haiku-*` 壳 ID 池中分配，每个壳 ID 映射到一个真实上游模型，并在 Science UI 中显示真实模型名。
...
| 通义千问 Qwen | OpenAI Chat Completions 兼容端点 | 由 CSP 代理转换为 Science 需要的 Anthropic 格式 |
...
CSP 内置了只读的 capability catalog，用来把 provider、工具调用、MCP/skill、Science 版本和 transport 的已知兼容性边界显式化。运行时 `status` 诊断会返回当前 profile 命中的 catalog 规则和固定边界规则，便于定位「当前配置为什么这样处理」以及「哪些能力只能诊断或降级」。
...
- 第三方 API Key 保存在 `~/.csp/CSP.json`，文件权限为 `0600`（首次启动会自动从旧路径 `~/.csswitch/config.json` 迁移）。
...
CSP 不是 Claude 官方服务，也不会让本地生成的启动门票获得 Anthropic 账号权限。以下限制是当前架构边界：
...
- Science 启动时仍可能访问内置的 profile、account 或服务发现接口；CSP 会在第三方模型模式下尽量把这些请求隔离或快速失败，因此本文不使用「完全不接触 Anthropic」这类绝对表述。
