...
让 [Claude Science](https://claude.com)（AI Jupyter 体验）的模型推理走第三方 API，保留 Science 那套交互（工具调用、Skill、MCP、代码执行、Jupyter），把底层模型换成你**自选的第三方 API**：DeepSeek、通义千问，或任意 OpenAI 兼容端点。类比 CC Switch 之于 Claude Code。
