...
> **v2.2.0** — 沙箱 SSH config Include 桥接；严格模型路由（未知 shell 显式报错）；Kimi K3 relay orphan tool_use/tool_result 清理；Gemini `role:"model"` 归一化 + safety finish_reason 可见标记。详见 [更新日志](./CHANGELOG.md)。
...
- 在隔离环境里启动 Claude Science；沙箱 HOME 的 `.ssh/config` 通过 `Include` 指令桥接到真实 `~/.ssh/config`（v2.2.0+），无需复制密钥即可在沙箱内使用 git/MCP over SSH。
