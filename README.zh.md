...
> **v1.7.1** — OpenAI-custom 长会话 / Resume 的 120s stream idle 修复（可计入的空 `text_delta` 保活）；Skill 扫描导入 `description: >-` 解析。详见 [Releases](https://github.com/counterfactual5/Claude-Science-Proxy/releases/tag/v1.7.1)。
...
- 管理本地 **Skills** 与 **stdio MCP 连接器**，启动时把已启用项部署进沙盒。
...
- 本地 **Skills 管理**：新建、导入、启用/停用、扫描导入，或从 Science 工作区「采纳」草稿；内置 **`csp-environment`** 环境手册；启用的 Skill 会在启动时部署进沙盒。
- 本地 **stdio MCP 管理**：新增/编辑，或从其它 AI 客户端「扫描导入」本地 MCP 连接器（如 Notion）；启用的连接器会在启动时部署进沙盒。
...
- Anthropic 托管的远程 MCP 服务不可用（`*.mcp.claude.com`）；**本地 stdio MCP 连接器可用**（见 MCP 标签页）。
