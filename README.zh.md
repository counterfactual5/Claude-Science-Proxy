...
> **v1.6.0** — Skills 页主按钮 **新建** 可直接编写 `SKILL.md`（导入移入 `⋯`）。代理在 system prompt 注入常驻 `host.mcp("web-search", …)` 指引，避免裸调 `web_search` 触发 OPERON not-found；并说明应用 `data["results"]` 解析返回。MCP 脚本覆写会标记 deploy 变更并触发沙盒重启。延续 **v1.5.0** 的内置 web-search MCP 与 `csp-web-access` Skill。问题与建议请走 [GitHub Issues](https://github.com/counterfactual5/Claude-Science-Proxy/issues)。
...
- 本地 **Skills 管理**：导入、启用/停用，或从常见 Agent 目录「扫描导入」含 `SKILL.md` 的 Skill；启用的 Skill 会在启动时部署进沙盒。
