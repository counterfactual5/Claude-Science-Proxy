...
> **v1.8.1** — Skills 支持本地目录 / zip / URL 导入；工作区采纳全屏预览；MCP 远程连接（sse / streamable_http）+ 扫描导入配置预览；扩展 HOME 级扫描根（WorkBuddy、Factory、Kimi Code、MiniMax 等）。详见 [Releases](https://github.com/counterfactual5/Claude-Science-Proxy/releases/tag/v1.8.1) · [更新日志](./CHANGELOG.md)。
...
- 管理本地 **Skills** 与 **MCP 连接器**（stdio + 远程），启动时把已启用项部署进沙盒。
...
- 本地 **Skills 管理**：新建；从目录 / zip / URL 导入；启用/停用；从其它 Agent 扫描导入；从 Science 工作区「采纳」草稿（全屏预览）；内置 **`csp-environment`** 环境手册；启用的 Skill 会在启动时部署进沙盒。
...
- 配置和日志都保存在本机，便于自查和反馈；MCP 连接器清单在 `~/.csp/mcp/inventory.json`（`0600`）。
...
- Anthropic 托管的远程 MCP 服务不可用（`*.mcp.claude.com`）；**本地 stdio 与自定义远程 MCP 连接器可用**（见 MCP 标签页）。
- Anthropic 托管的顶层 **`web_search` / `web_fetch` 不可用**；请用内置 `web-search` MCP：`host.mcp("web-search", "csp_web_search" | "search_literature" | "fetch_url", …)`，结果取 `data["results"]`。
- 依赖真实 Claude 账号授权的目录连接器、远程插件、云端能力可能会显示 session expired 或 unavailable。
- **OpenAI 兼容类配置**（自定义 OpenAI / OpenAI Responses，含 GLM 的 OpenAI Chat 地址）：**v1.7.1** 起长会话与 **Resume** 的 120s `stream idle` 误杀已修复；上游 **429 / 公平使用限流** 或极慢响应仍可能触发重试，见 [已知问题](./docs/known-issues.md#openai-custom-streaming)。
- 当前 macOS 包尚未 Apple 公证；代理仍依赖 `python3`。

完整列表见 [`docs/known-issues.md`](./docs/known-issues.md)。已知问题和故障请前往 GitHub Issues 进行反馈。

## 参与贡献

欢迎 issue 与 PR。请先读 [`CONTRIBUTING.md`](./CONTRIBUTING.md)，再读 [`AGENT.md`](./AGENT.md) 与 [`docs/DEVELOPMENT.md`](./docs/DEVELOPMENT.md)。安全漏洞请走 [`SECURITY.md`](./SECURITY.md) 的私有上报渠道，勿开公开 issue。提交前跑 `bash test/run_all.sh`。
