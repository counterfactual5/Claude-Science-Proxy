...
> **v1.4.1** — Skill 每行操作改为紧凑的 `⋯` 菜单（编辑 / 打开文件夹 / 删除）。延续 **v1.4.0** 的 **从 Science 采纳**：当虚拟登录无法调用 `host.skills` 发布时，可将 Science 在工作区写下的 Skill 草稿（`SKILL.md` 文件夹、`kernel.py` 等）导入 `~/.csp/skills/`。问题与建议请走 [GitHub Issues](https://github.com/counterfactual5/Claude-Science-Proxy/issues)。
