...
> **v1.5.0** — 内置免费 **web-search MCP**（无需 API Key），并新增常驻 **`csp-web-access`** Skill，向 Claude Science 讲清本地沙盒约定：优先使用本地 web-search 而非托管的 `web_search` 工具；产物写入工作区并调用 `save_artifacts([...])`（本地不存在 `/mnt/data`）；绘制中日韩标签前设置可用的 matplotlib 字体。延续 v1.4.x 的紧凑 `⋯` 行操作菜单与 **从 Science 采纳**。问题与建议请走 [GitHub Issues](https://github.com/counterfactual5/Claude-Science-Proxy/issues)。
