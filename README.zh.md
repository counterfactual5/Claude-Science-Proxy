...
> **v1.3.1** 修复使用 `#!/usr/bin/env node` 的 npm 全局 MCP 启动脚本（如 `notion-mcp-server`），让它们在 Science 沙盒中不依赖宿主机 `PATH` 也能启动。问题与建议请走 [GitHub Issues](https://github.com/counterfactual5/Claude-Science-Proxy/issues)。
