...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.6.6-brightgreen.svg" alt="v1.6.6">
...
> **v1.6.6** — 通用搜索对外统一为 MCP 方法名 `csp_web_search`（`tools/list` 不再并列 `web_search`）；内部仍接受未列出的 `web_search` 分发别名。Anthropic 原生 `web_search` 仍不可调用。本地构建，暂不发 Release。
