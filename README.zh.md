...
> **v1.6.1** — 启动时自动为内置 `web-search` 全部提供方（DuckDuckGo / Wikipedia / Brave / Serper / Tavily）写入 Science 网络授权；可用 `~/.csp/network-allowlist.json` 扩展（MCP `⋯` → **网络授权配置**）。延续 **v1.6.0** 的 Skills **新建**、代理 `host.mcp` 指引与日期注入。问题与建议请走 [GitHub Issues](https://github.com/counterfactual5/Claude-Science-Proxy/issues)。
