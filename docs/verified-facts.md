...
5. **CC Switch 的代理本身就是完整翻译器**：二进制含 `/v1/messages`、`/v1/chat/completions`、`cc_switch_transform_error`、两套协议字段与 SSE 桥接，内建模型目录含 DeepSeek/Qwen/Kimi。端口默认 `127.0.0.1:15721`，`proxy_config` 的 `app_type` 只允许 `claude/codex/gemini`（Science 复用 `claude`）。
...
- 待决：最终用自研 `qwen_proxy.py`（透明可控）还是复用 CC Switch 内建代理（覆盖面广）。
