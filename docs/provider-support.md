...
**关键启示（对 CSP 直接有用）**：cc-switch 的预设表证明**大量国产 provider 都已提供 `/anthropic` 端点**，这些对我们就是「透传」一条路，几乎零成本。从其预设里能确认的国产 Anthropic 端点（并入下表核对）：
...
**和我们的差异**：cc-switch 换的是 **Claude Code**（配置即用、无强制 OAuth）；CSP 换的是 **Claude Science**（强制 OAuth 门票 → 我们还要伪造虚拟登录 + 起隔离沙箱）。所以**「有没有 Anthropic 端点」这一列可以直接借鉴 cc-switch，但门票机制我们独有**。
