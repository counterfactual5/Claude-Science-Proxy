...
- **内部 IPC 保持不动**：`CSSWITCH_*` 环境变量、`csswitch_proxy.py` 文件名、`CSSWITCH_REPO`（开发态指定仓库根）。
...
- **配置读写**：`get_config`（→ `{profiles, templates, active_id, proxy_port, sandbox_port, pending_notice}`，key 掩码）、`create_profile`、`update_profile_connection`（改 base_url/model/key）、`update_profile_metadata`（改名/备注）、`delete_profile`、`clear_profile_key`、`set_active_profile`（设为当前，经切换事务）、`set_settings`。
