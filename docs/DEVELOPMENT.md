...
- 用户数据目录：`~/.csp/`（`CSP.json`、`logs/`、`sandbox/home`）；首次启动从 `~/.csswitch/config.json` 一次性迁移。
...
| `~/.csswitch`、`config.json` | 一次性迁移源路径（`config.rs` `LEGACY_*`） |
| `com.csswitch.maintenance` | 已 DEPRECATED；`install-maintenance.sh` 安装时卸载 |
| 测试临时目录前缀 `csswitch-*`、`.csswitch` | 单测隔离目录名，非用户路径 |
| `findings/`、`CHANGELOG` 旧版本条目 | 历史证据，见 [`../findings/README.md`](../findings/README.md) |
| `scripts/doctor.sh` 读旧 config | 迁移期回退：新装无 `CSP.json` 时仍可读 legacy 文件 |
...
**能力面**：
- **多 profile 配置管理**（cc-switch 式）：7 家 provider 模板（DeepSeek / 通义千问 / 智谱 GLM / Kimi / MiniMax / 小米 MiMo / 硅基流动 / OpenRouter）+ 自定义端点；同一家可保存多套（不同 key / 模型）；JSON 存储 `~/.csp/CSP.json`（schema v4，含 v1→v2→v3→v4 一次性迁移），key 明文 0600、只回掩码。
...
- **UI**：正常窗口 420×700（`decorations:true`，已去托盘/菜单栏），配置列表 + 多选模型；同一时间仅一条 profile 生效。
...
desktop/src-tauri/src/
  lib.rs           tauri command + 切换事务 + launcher（起代理/沙箱、注入 env）+ 诊断 status
...
- **运行控制**：`stop_all`、`one_click_login`（→ `{url, msg_key, msg_vars, action}`）、`status`（诊断快照，可选）。
...
# 切千问：--provider qwen + DASHSCOPE_API_KEY；relay 家：--provider relay + CSP_RELAY_BASE_URL/KEY/MODEL
# 也支持 --env-file
