...
- **Hosted `web_search` is unavailable; use `repl` → `host.mcp`.** Under CSP virtual login the Anthropic-hosted `web_search` / `web_fetch` tools do not exist in OPERON's toolset and fail with `Tool 'web_search' not found on agent` if called bare. Local MCP tools are **not** top-level model tools. Search/fetch via the `repl` tool:
  - `host.mcp("web-search", "search_literature", query="...", max_results=N)` (aliases: `"web_search"`, `"csp_web_search"`)
  - then `host.mcp("web-search", "fetch_url", url="...")` when needed
...
- `host.mcp("web-search", "search_literature"|"web_search"|"csp_web_search", query=..., max_results=5, provider="auto")` — scholarly / web search.
- `host.mcp("web-search", "fetch_url"|"web_fetch", url=..., max_chars=8000)` — fetch a page as readable text.
