...
- **Hosted `web_search` is unavailable.** Under CSP virtual login the Anthropic-hosted `web_search` tool does not exist and fails with `Tool 'web_search' not found on agent` if something still routes to it. The local **`web-search`** MCP now advertises `web_search` / `web_fetch` (plus `search_literature` / `csp_web_search` / `fetch_url`) so model-native calls resolve locally. Egress is a scholarly allowlist (Crossref / arXiv / PubMed / OpenAlex / Semantic Scholar / Notion / PyPI reliable; general and paid search engines usually blocked). See [Built-in `web-search` connector](#local-mcp).
...
**Built-in `web-search` connector (free, no API key).** Because Claude Science lacks Anthropic's hosted `web_search` tool under CSP's virtual login (the sandbox log shows `Tool 'web_search' not found on agent 'OPERON'`), CSP ships a bundled multi-provider search + fetch MCP server and **seeds it into the inventory on first run**, enabled by default. It is a small self-contained **Python** stdio server (`web_search_server.py`, bundled via `include_str!` and written to `$SANDBOX_HOME/.claude-science/mcp/csp-web-search-server.py` at deploy time, like the Node shim). Python is chosen deliberately: unlike the Node/axios stacks, Python's `requests`/`urllib` honour the injected `HTTPS_PROXY` and issue a proper `CONNECT` tunnel, so it needs no shim. The connector's interpreter is resolved to the sandbox's own `claude-science-mcp` conda env Python (falling back to the `python` env, then `python3`) and re-resolved on every deploy so it self-heals. Its inventory description is also refreshed on each launch / deploy for already-seeded users. It exposes:

- `web_search(query, max_results=5, provider="auto")` — **advertised** local search tool (same schema as the aliases). Under CSP virtual login there is no Anthropic-hosted `web_search`, so advertising this name is safe and correct: models that reflexively call `web_search` (e.g. glm-4.7) resolve here instead of failing with `Tool 'web_search' not found on agent`.
- `search_literature(...)` / `csp_web_search(...)` — identical aliases of `web_search` (belt-and-suspenders).
- `fetch_url(url, max_chars=8000)` / `web_fetch(...)` — fetch a page and return readable text (HTML stripped); `web_fetch` is an advertised alias of `fetch_url`.

**Why `web_search` is advertised again.** An earlier rename hid `web_search` from `tools/list` out of caution about clashing with Anthropic's hosted tool. Under CSP virtual login that hosted tool **does not exist**, so the rename was over-cautious: models still call `web_search` by reflex, hit Operon with `Tool 'web_search' not found on agent`, and never reach the local MCP. Standing-skill guidance cannot reliably stop that reflex (progressive disclosure). The fix is to **re-advertise** `web_search` (and `web_fetch`) in the local connector's `tools/list` so those calls resolve locally. Rebuild the app and **Start Claude Science** (new conversation) to pick up the new tools/list — in-session MCP tool lists do not hot-reload.
