...
Scholarly baselines (Crossref / arXiv / PubMed / …) remain Science-builtin and need no CSP grant. Do **not** put HTML `duckduckgo` into `auto` until scraping is reliable — `duckduckgo_ia` is the safer general-web candidate once domains are granted. `provider=auto` still prefers scholarly no-key defaults, then key providers when keys exist. A user who disables/removes the built-in connector is respected via `~/.csp/mcp/.seeded-web-search`.
