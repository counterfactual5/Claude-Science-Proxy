...
2. **User extensions** from `~/.csp/network-allowlist.json` (`{ "version": 1, "domains": ["example.com"] }`). Create/open via MCP tab `⋯` → **网络授权配置**. Hostname-only entries; Stop → Start after edits (Operon does not hot-reload grants).
