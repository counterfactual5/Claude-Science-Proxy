...
1. **Built-in** domains for every bundled `web-search` provider: `html.duckduckgo.com`, `api.duckduckgo.com`, `en.wikipedia.org`, `api.search.brave.com`, `google.serper.dev`, `api.tavily.com` — so configuring a Brave/Serper/Tavily API key in the MCP tab works without a separate manual grant.
...
Scholarly baselines (Crossref / arXiv / PubMed / …) remain Science-builtin and need no CSP grant. **Two search lanes (v1.6.3+):** GENERAL (`web_search` / `csp_web_search`) auto = keyed Brave/Serper/Tavily → `duckduckgo_ia`; LITERATURE (`search_literature`) auto = wikipedia → Crossref → arXiv → PubMed. HTML `duckduckgo` stays out of both autos. Proxy standing guidance mirrors the split so product/news queries use the general lane. A user who disables/removes the built-in connector is respected via `~/.csp/mcp/.seeded-web-search`.
