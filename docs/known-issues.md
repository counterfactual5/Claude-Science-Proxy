...
- **根因（`proxy/csp_proxy.py` PROVIDERS 注释已证实，逆向标记 s0/ZjO/XjO/hB_）**：Science 模型面板有**二进制写死的两道硬规则**——① 可选模型 id **必须 `claude-` 开头**；② 只有 `claude-{opus|sonnet|haiku}-<纯数字版本>` 进**主列表**（每 family 留一个），其余进「More models」折叠。**CSP 改不了 Science 这个 UI**，只能控制代理 `/v1/models` 返回什么。
