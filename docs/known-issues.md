...
Scholarly baselines (Crossref / arXiv / PubMed / …) remain Science-builtin and need no CSP grant. **Two search lanes (v1.6.3+; free GENERAL fallbacks expanded in v1.6.5):** GENERAL — `web_search` and `csp_web_search` are **aliases of the same method** (not two engines); auto = optional keyed Brave/Serper/Tavily → `duckduckgo_ia` → `duckduckgo_lite` → `wikipedia` (no key required; empty Instant Answer is common and ≠ missing key). LITERATURE — `search_literature`; auto = wikipedia → Crossref → arXiv → PubMed. Full HTML `duckduckgo` stays out of auto (anti-bot). CSP also pre-grants `lite.duckduckgo.com`. Proxy + `csp-environment` standing guidance mirror this so product/news queries use the general lane without inventing an API-key requirement. A user who disables/removes the built-in connector is respected via `~/.csp/mcp/.seeded-web-search`.
