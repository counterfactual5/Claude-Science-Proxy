...
> GitHub Releases / [`known-issues.md`](known-issues.md) / [`findings/README.md`](../findings/README.md) 为准。本稿仅留作首发规划的历史留存。
...
```
让 Claude Science 的推理走自选的第三方 API（DeepSeek / GLM / Kimi / 任意 OpenAI 或 Anthropic 兼容端点），
保留工具调用·Skill·MCP·代码执行，含本地虚拟登录。macOS 桌面 app（Tauri）。
```
...
```
claude  claude-science  anthropic  deepseek  glm  kimi  minimax
openai-compatible  llm  proxy  mcp  tauri  macos  apple-silicon
```
