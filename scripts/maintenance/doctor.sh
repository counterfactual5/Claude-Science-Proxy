...
# node is only needed when running scripts/oauth/make-virtual-oauth.mjs standalone (dev/parity); missing node is a warning, not a failure.
if command -v node >/dev/null 2>&1; then pass "node $(node --version 2>&1)（app 已不需要，仅 dev 脚本用）"; else warn "无 node（app 无需；仅独立跑 scripts/oauth/make-virtual-oauth.mjs 时才需要）"; fi
