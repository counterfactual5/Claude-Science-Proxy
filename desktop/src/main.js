...
const I18N = {
  cn: {
    ...
    errPlatterTooMany: "拼盘最多 {max} 个模型。",
    errPlatterMissingModel: "拼盘条目缺少模型 ID。",
    errPlatterAdapterUnsupported: "「{name}」无法加入拼盘（请先填写 API Key）。",
    errSandboxStopBeforeRelaunchFailed: "重启前无法停止旧沙箱：{error}。请手动停止后重试（真实实例 8765 未受影响）。",
    platterSaveOk: "拼盘已保存。",
  ...
  intl: {
    ...
    errPlatterTooMany: "Platter supports at most {max} models.",
    errPlatterMissingModel: "A platter entry is missing its model ID.",
    errPlatterAdapterUnsupported: "「{name}」 cannot join the platter (API key required).",
    errSandboxStopBeforeRelaunchFailed: "Could not stop the old sandbox before relaunch: {error}. Stop it manually and retry (real instance on 8765 unaffected).",
