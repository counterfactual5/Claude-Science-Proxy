...
Claude Science（沙箱 · 虚拟登录；登录仅当启动门票，推理不走 Anthropic）
   ...
   ▼
翻译代理 proxy/csp_proxy.py（默认 deepseek 原生透传 / qwen 翻译，可切）
...
- **发布态 / 待办**：已公开于 github.com/SuperJJ007/Claude-Science-Proxy；桌面 app 在 `desktop/`（Tauri **正常窗口**进程管家，已去托盘）。**当前 Latest 版本以 GitHub Releases / [`CHANGELOG.md`](CHANGELOG.md) 为准**；**待办队列与排期** → [`docs/known-issues.md`](docs/known-issues.md)。（此处不写具体版本号与「当前最优先」等易变状态，那些归 known-issues 与项目记忆 `memory/`。）
- **对外文案（脱敏）**：用户**可见**文案不露骨，别直说「越过 / 绕过登录」；主按钮用「一键开始」类中性说法。技术**内部**文档描述机制时可仍用「越过门票」。详见 `docs/known-issues.md` 第 1 条。
- **上游/模型**：默认 DeepSeek（原生 Anthropic 透传），可 `--provider qwen`（翻译）。模型映射与选择器广告 id 见 `csp_proxy.py` 的 `PROVIDERS`。
...
```
proxy/csp_proxy.py           【主】provider 可切代理（deepseek 默认 / qwen）
...
- 起代理（默认 DeepSeek）：`DEEPSEEK_API_KEY=... python3 proxy/csp_proxy.py --provider deepseek --port 18991`（切千问用 `--provider qwen` + `DASHSCOPE_API_KEY`；也支持 `--env-file`）
