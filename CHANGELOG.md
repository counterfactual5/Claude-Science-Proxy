...
### Fixed
- **Local `web-search` MCP re-advertises `web_search` / `web_fetch`**: The earlier rename that hid `web_search` from `tools/list` was over-cautious — under CSP virtual login there is **no** Anthropic-hosted `web_search`, so models (e.g. glm-4.7) that reflexively call that name failed with `Tool 'web_search' not found on agent 'OPERON'` instead of reaching the local connector. The bundled server (v1.2.0) now advertises **`web_search`**, **`search_literature`**, **`csp_web_search`**, **`fetch_url`**, and **`web_fetch`** in `tools/list` (all search names share one handler; fetch aliases share another). Standing-skill guidance alone cannot stop the reflex (progressive disclosure). Already-seeded users get the updated connector description via `refresh_builtin` on launch and a deploy-time description rewrite (command/args were already self-healing). Rebuild + **Start Claude Science** (new conversation) is required — in-session tools/list does not hot-reload.
- **Built-in `web-search` MCP tool-name conflict with the hosted `web_search`** *(superseded by the Fixed entry above)*: An interim build advertised only distinct names (`search_literature` / `csp_web_search` / `fetch_url`) with `web_search` as a hidden dispatch alias. That approach is reverted for CSP virtual login where no hosted tool exists.
...
- **Built-in `web-search` MCP connector (free, no API key)**: CSP now ships a bundled multi-provider web search + page fetch MCP server and seeds it into `~/.csp/mcp/inventory.json` on first run, enabled by default, so Claude Science has real `web_search`/`fetch_url` despite Anthropic's hosted `web_search` being unavailable under CSP virtual login. It is a self-contained **Python** stdio server (`web_search_server.py`, bundled via `include_str!` and deployed next to the Node shim); Python is used because its `requests`/`urllib` honour the injected `HTTPS_PROXY` and `CONNECT`-tunnel correctly, needing no shim. The interpreter is resolved to the sandbox's own Python and re-resolved on every deploy so the entry self-heals.
  - **Tools**: `web_search(query, max_results=5, provider="auto")` (aliases `search_literature`, `csp_web_search`) returns structured results; `fetch_url` / `web_fetch` return readable page text. Under CSP, advertising `web_search` is safe — there is no hosted clash.
