...
- **`host.mcp` search return-shape guidance (fixes `AttributeError: 'str' object has no attribute 'get'`)**: After the proxy injection got Science calling `host.mcp("web-search", …)` correctly, the model often treated the return as a list of hit dicts and did `for r in enumerate(results): r.get("title")`. Live `host_call_log` / execution logs show the tool already returns pretty-printed JSON that Science parses into a **dict** `{"provider","query","results":[…],"warnings"}` — iterating that dict yields **string keys**, hence the AttributeError. Tool descriptions (`web_search_server.py` v1.3.0), inventory description (`builtin.rs`), proxy standing guidance, and `csp-web-access` Skill now document: `data = host.mcp(...); hits = data["results"]` (or `print(data)`). Fetch returns `{"url","status","content"}`.
